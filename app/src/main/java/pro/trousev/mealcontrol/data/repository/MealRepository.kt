...
class MealRepository(
    ...
    suspend fun getAllMeals(): List<MealWithComponents> = mealDao.getAllMealsWithComponents()

    suspend fun getMealById(mealId: Long): MealWithComponents? = mealDao.getMealWithComponents(mealId)

    private fun mapComponentsToEntities(
        components: List<Triple<String, Double, List<Number>>>,
        mealId: Long,
    ): List<MealComponentEntity> =
        components.map { (name, weight, values) ->
            MealComponentEntity(
                mealId = mealId,
                name = name,
